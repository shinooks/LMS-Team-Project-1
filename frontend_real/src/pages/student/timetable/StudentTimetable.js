...
const StudentTimetable = ({ }) => {
  ...
  const fetchEnrollmentData = async () => {
    try {
      ...
      console.log("가져온 데이터 : " + enrollmentData);
              ...
              {timeSlots.map((time, timeIndex) => (
                  <div key={`row-${timeIndex}`} className="contents">
                    ...
                    {days.map((day, dayIndex) => {
                      const cell = timeTableData[timeIndex]?.[dayIndex]; // 안전하게 접근
                      return (
                          <div
                          ...
                          >
                            {cell ? (
                                ...
                                <div className="text-xs text-gray-400"></div> // 빈 경우 처리
                            )}
                          </div>
                      );
                    })}
